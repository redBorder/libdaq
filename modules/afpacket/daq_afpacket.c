...
static uint32_t count_queued_packets_for_context(AFPacket_Context_t *context) {
    ...
    for (instance = context->instances; instance; instance = instance->next) {
        if (instance->active) {
            total += count_queued_packets(instance);
        }
    }
    return total;
}
...
static void update_soft_bypass_status(AFPacket_Context_t *context) {
    uint32_t num_queued_packets = count_queued_packets_for_context(context);
...
static int bind_instance_interface(AFPacket_Context_t *afpc, AFPacketInstance *instance, int protocol)
{
    ...
    {
        SET_ERROR(afpc->modinst, "%s: getsockopt: %s", __func__, err ? strerror(err) : strerror(errno));
        return DAQ_ERROR;
    }
...
static int set_up_ring(AFPacket_Context_t *afpc, AFPacketInstance *instance, AFPacketRing *ring)
{
    ...
    if (!ring->entries)
    {
        SET_ERROR(afpc->modinst, "%s: Could not allocate ring buffer entries for device %s!", __func__, instance->name);
        return DAQ_ERROR_NOMEM;
    }
...
static AFPacketInstance *create_instance(AFPacket_Context_t *afpc, const char *device)
{
    ...
    {
        SET_ERROR(afpc->modinst, "%s: Could not find index for device %s", __func__, instance->name);
        goto err;
    }
...
static int create_ring(AFPacket_Context_t *afpc, AFPacketInstance *instance, AFPacketRing *ring, int optname)
{
    ...
    for (int order = DEFAULT_ORDER; order >= 0; order--)
    {
        ...
        if (rc)
        {
            ...
            SET_ERROR(afpc->modinst, "%s: Couldn't create kernel ring on packet socket: %s",
    ...
    SET_ERROR(afpc->modinst, "%s: Couldn't allocate enough memory for the kernel packet ring!", instance->name);
    return DAQ_ERROR;
}
...
static int mmap_rings(AFPacket_Context_t *afpc, AFPacketInstance *instance)
{
    ...
    if (instance->buffer == MAP_FAILED)
    {
        SET_ERROR(afpc->modinst, "%s: Could not MMAP the ring: %s", __func__, strerror(errno));
        return DAQ_ERROR;
    }
...
static int configure_fanout(AFPacket_Context_t *afpc, AFPacketInstance *instance)
{
    ...
    {
        SET_ERROR(afpc->modinst, "%s: Could not configure packet fanout: %s", __func__, strerror(errno));
        return DAQ_ERROR;
    }
...
static int afpacket_daq_instantiate(const DAQ_ModuleConfig_h modcfg, DAQ_ModuleInstance_h modinst, void **ctxt_ptr)
{
    ...
    if (!afpc)
    {
        SET_ERROR(modinst, "%s: Couldn't allocate memory for the new AFPacket context!", __func__);
    ...
    while (varKey)
    {
        ...
        else if (!strcmp(varKey, "fanout_type"))
        {
            if (!varValue)
            {
                SET_ERROR(modinst, "%s: %s requires an argument!", __func__, varKey);
                goto err;
            }
    ...
    if (afpc->sw_bypass.upper_threshold == 0 || afpc->sw_bypass.lower_threshold == 0) {
        afpc->sw_bypass.sampling_rate = 0;
        afpacket_debug(afpc, "Software bypass disabled (thresholds not set)\n");
    } else {
        afpacket_debug(afpc, "Software bypass configured - upper: %lu, lower: %lu, sampling: %lu\n",
                      afpc->sw_bypass.upper_threshold, afpc->sw_bypass.lower_threshold,
                      afpc->sw_bypass.sampling_rate);
        if (afpc->sw_bypass.software_bypass_log) {
            afpacket_debug(afpc, "Bypass logging enabled to: %s\n", afpc->sw_bypass.software_bypass_log);
        }
    }
    uint32_t size;
...
static int afpacket_daq_set_filter(void *handle, const char *filter)
{
    ...
    if (!afpc->filter)
    {
        SET_ERROR(afpc->modinst, "%s: Couldn't allocate memory for the filter string!", __func__);
        return DAQ_ERROR;
    }
...
static int afpacket_inject_packet(AFPacket_Context_t *afpc, AFPacketInstance *egress, const uint8_t *data, uint32_t data_len)
{
    ...
    if (rval != DAQ_SUCCESS)
    {
        if (rval == DAQ_ERROR_AGAIN)
            ...
            SET_ERROR(afpc->modinst, "%s: Error sending packet: %s (%d)", __func__, strerror(errno), errno);
        return rval;
    }
